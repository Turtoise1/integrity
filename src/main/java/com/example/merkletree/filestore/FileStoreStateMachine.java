 ...
 */
package com.example.merkletree.filestore;
...
public class FileStoreStateMachine extends BaseStateMachine {
    ...
    public CompletableFuture<ByteString> read(LogEntryProto entry, TransactionContext context) {
        ...
        final WriteRequestHeaderProto h = proto.getWriteHeader();
        CompletableFuture<ExamplesProtos.ReadReplyProto> reply = files.read(h.getPath().toStringUtf8(), h.getOffset(),
                h.getLength(), false);
    ...
    private CompletableFuture<Message> delete(long index, DeleteRequestProto request) {
        ...
        return files.delete(index, path)
                .thenApply(resolved -> Message.valueOf(DeleteReplyProto.newBuilder().setResolvedPath(
